...
function main() {
  ...
  const embeddedRuntime = {
    ...
    pythonPath: ["python/site-packages", "backend"],
    ...
    sourcePython: pythonInfo.executable,
    sourcePythonHome: pythonHomeSource,
    sourceSitePackages: sitePackagesSource,
    sourceLlamaBinDir: process.env.CHAOSENGINE_LLAMA_BIN_DIR || defaultLlamaBinDir(),
...
function stageLlamaBinaries() {
  ...
  if (!fs.existsSync(sourceDir)) {
    if (strict) {
      throw new Error(`llama.cpp binary directory not found: ${sourceDir}`);
    }
    return [`llama.cpp binary directory not found: ${sourceDir}`];
  }
  ...
  if (!selected.includes(binaryName("llama-server"))) {
    const message = `Missing ${binaryName("llama-server")} in ${sourceDir}`;
    if (strict) {
